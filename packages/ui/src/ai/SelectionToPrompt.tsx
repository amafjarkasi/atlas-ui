...
export function SelectionToPrompt({ text, onAction, actions = ['Explain', 'Summarize', 'Translate', 'Fix'], previewMaxChars = 240 }: SelectionToPromptProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: 10, borderRadius: 10, borderWidth: 1, borderColor: border.subtle, backgroundColor: surface.card }}>
      <text style={{ fontSize: 11, fontWeight: 600, color: t.muted, fontFamily: FONT }}>SELECTED TEXT</text>
